...
export default function App() {
  ...
  const handleLogin = () => {
    const user = allAccounts.find(u => 
        (normalize(u.name) === normalize(authForm.name) || u.email?.toLowerCase() === authForm.name.toLowerCase()) 
        && u.password === authForm.pass
    );
  ...
  const handleSignupInit = async (role: "patient" | "doctor") => {
    ...
    try {
        ...
        } else { alert("Could not send email. Check Server console."); }
    } catch (err) { alert("Server Error."); }
    finally { setIsVerifying(false); }
  };

  const handleForgotPassword = async () => {
    const userExists = allAccounts.find(u => u.email?.toLowerCase() === recoveryEmail.toLowerCase());
    if (!userExists) return alert("No account found with this email.");
  ...
  const handleVerifyOTP = async (isRecovery: boolean) => {
    ...
    try {
        ...
        if (res.ok) {
            if (isRecovery) {
                const user = allAccounts.find(u => u.email.toLowerCase() === recoveryEmail.toLowerCase());
                setActiveUser(user);
    ...
    finally { setIsVerifying(false); setOtpInput(""); }
  };
  ...
  const handleMedResponse = (action: "taken" | "skip" | "snooze") => {
    if (action === "taken") {
      ...
      setEmergencyAlert(`ALERT: dose of ${activeReminder.name} skipped. SMS to ${activeUser.emergencyName}.`);
    ...
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 uppercase font-black">
        ...
        <div className="bg-white p-12 rounded-[40px] w-full max-w-md shadow-2xl space-y-8">
            ...
            <div className="space-y-4">
                ...
                <button onClick={() => setAppStep("forgot-email")} className="w-full text-xs text-indigo-600 font-bold">FORGOT PASSWORD?</button>
            </div>
        </div>
    </div>
  );
        ...
        <div className="bg-white p-10 rounded-[40px] w-full max-w-md shadow-2xl space-y-6">
            ...
            <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-2 pt-4">
                    <button disabled={isVerifying} onClick={() => handleSignupInit("patient")} className="w-full py-4 bg-indigo-50 text-indigo-700 border-2 border-indigo-200 rounded-2xl flex justify-between px-8 uppercase">{isVerifying ? "Sending..." : "As Patient"} <User size={20}/></button>
  ...
  if (authStep === "forgot-email") return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 uppercase font-black">
        <div className="bg-white p-12 rounded-[40px] w-full max-w-md shadow-2xl text-center space-y-6">
            ...
            <input placeholder="Registered Gmail" className="w-full p-5 bg-slate-50 border rounded-2xl outline-none" onChange={e => setRecoveryEmail(e.target.value)} />
            <button disabled={isVerifying} onClick={handleForgotPassword} className="w-full py-5 bg-indigo-600 text-white rounded-2xl shadow-lg">{isVerifying ? "Sending..." : "Get OTP"}</button>
            <button onClick={() => setAppStep("login")} className="text-xs text-slate-400">Back</button>
        </div>
    </div>
  );
  ...
  if (authStep === "forgot-otp") return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 uppercase font-black">
        <div className="bg-white p-12 rounded-[40px] w-full max-w-md shadow-2xl text-center space-y-8">
            ...
            <button onClick={() => handleVerifyOTP(true)} className="w-full py-5 bg-indigo-600 text-white rounded-2xl shadow-lg">Verify</button>
        </div>
    </div>
  );
  ...
  if (authStep === "recover-choice") return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 uppercase font-black text-center">
        <div className="bg-white p-12 rounded-[40px] w-full max-w-md shadow-2xl space-y-8">
            ...
            <h2 className="text-2xl">Identity Verified</h2>
            <div className="space-y-4">
                <button onClick={() => setAppStep("reset-pass")} className="w-full py-5 bg-indigo-600 text-white rounded-2xl flex items-center justify-center gap-3"><KeyRound size={18}/> New Password</button>
  ...
  if (authStep === "reset-pass") return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 uppercase font-black">
        <div className="bg-white p-12 rounded-[40px] w-full max-w-md shadow-2xl text-center space-y-6">
            ...
            <input type="password" placeholder="New Password" className="w-full p-5 bg-slate-50 border rounded-2xl outline-none" onChange={e => setNewPassword(e.target.value)} />
            <button onClick={() => {
                const updated = allAccounts.map(u => u.email.toLowerCase() === recoveryEmail.toLowerCase() ? {...u, password: newPassword} : u);
                setAllAccounts(updated);
                setAppStep("dashboard");
                alert("Updated.");
            }} className="w-full py-5 bg-indigo-600 text-white rounded-2xl">Save & Continue</button>
        </div>
    </div>
  );
  ...
  if (authStep === "otp-verify") return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 uppercase font-black">
        <div className="bg-white p-12 rounded-[40px] w-full max-w-md shadow-2xl text-center space-y-8">
            ...
            <h2 className="text-2xl">Verify Gmail</h2>
            <p className="text-slate-400 text-xs italic">Sent to {activeUser.email}</p>
            <input maxLength={4} placeholder="0000" className="w-full p-6 bg-slate-50 border rounded-3xl text-center text-4xl outline-none border-indigo-200" onChange={e => setOtpInput(e.target.value)} />
            <button disabled={isVerifying} onClick={() => handleVerifyOTP(false)} className="w-full py-5 bg-indigo-600 text-white rounded-2xl shadow-lg">{isVerifying ? "Checking..." : "Verify"}</button>
        </div>
    </div>
  );
        ...
        <div className="bg-white p-10 rounded-[40px] w-full max-w-md shadow-xl space-y-6">
            ...
            {authStep === "setup-patient" ? (
              <>
                ...
                <div className="border-t pt-4 space-y-2 font-sans"><input placeholder="Doctor's Name" className="w-full p-4 bg-slate-50 border rounded-2xl" onChange={e => setSetupData({...setupData, doctorName: e.target.value})} /> <input placeholder="Doctor's Phone" maxLength={10} className="w-full p-4 bg-slate-50 border rounded-2xl" onChange={e => setSetupData({...setupData, doctorPhone: e.target.value})} /></div>
              </>
            ) : (
              <>
                ...
                <textarea placeholder="Hospital Address" className="w-full p-4 bg-slate-50 border rounded-2xl min-h-[120px]" onChange={e => setSetupData({...setupData, clinicAddress: e.target.value})} />
              </>
            )}
            <button onClick={finalizeAccount} className="w-full py-4 bg-indigo-600 text-white rounded-2xl shadow-lg uppercase">Finish Registration</button>
        </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col font-black uppercase">
      {/* (Keep rest of your Dashboard/Tab logic exactly same as previous turns) */}
      <AnimatePresence>
        {activeReminder && (
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 text-left uppercase">
        <div className="lg:col-span-3 flex flex-col gap-6 font-sans">
          <div className="bg-white rounded-3xl p-5 shadow-sm border space-y-1">
            <button onClick={() => setActiveTab("home")} className={`w-full flex items-center gap-3 px-6 py-4 rounded-2xl text-sm ${activeTab === 'home' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-50'}`}><Activity size={18}/> Dashboard</button>
          ...
          <AnimatePresence mode="wait">
            {activeTab === "home" && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                <div className="bg-indigo-900 text-white p-12 rounded-[50px] shadow-lg relative overflow-hidden text-left font-black">
                    <h2 className="text-4xl font-black mb-3">Hello, {activeUser.name}!</h2>
            ...
            {activeTab === "meds" && (
                <div className="space-y-8 text-left uppercase">
                    ...
                    <div className="grid grid-cols-1 gap-4 font-black">{(activeUser.medicines || []).map((m:any) => (<div key={m.id} className="p-8 bg-white border rounded-[35px] shadow-sm flex justify-between items-center uppercase font-black"><div className="text-left font-black"><p className="text-2xl font-black">{m.name}</p><div className="flex items-center gap-3 mt-2 font-sans"><div className="flex items-center gap-2 bg-indigo-50 px-4 py-2 rounded-full border border-indigo-100"><Clock size={14} className={!m.time ? "text-rose-500 animate-pulse" : "text-indigo-600"} /><input type="time" className="bg-transparent text-[10px] outline-none font-black uppercase" value={m.time || ""} onChange={(e) => setActiveUser({...activeUser, medicines: activeUser.medicines.map((x:any)=>x.id===m.id?{...x, time: e.target.value}:x)})} /></div><span className="text-[10px] text-slate-400 opacity-50 uppercase tracking-widest">{m.dosage}</span></div></div><button onClick={() => setActiveUser({...activeUser, medicines: activeUser.medicines.filter((x:any) => x.id !== m.id)})} className="p-4 text-rose-500 bg-rose-50 rounded-2xl hover:bg-rose-600 hover:text-white transition-all"><Trash2/></button></div>))}</div>
                </div>
            )}

            {activeTab === "apps" && (
                <div className="space-y-8 uppercase font-black text-left"><div className="bg-white p-12 rounded-[50px] border shadow-sm"><h3 className="text-2xl mb-8 tracking-widest uppercase">Book Appointment</h3><form onSubmit={bookAppointment} className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left font-black"><input required placeholder="Reason for Visit" className="p-6 bg-slate-50 border rounded-[30px] outline-none" value={apptForm.reason} onChange={e => setApptForm({...apptForm, reason: e.target.value})} /><input required type="datetime-local" className="p-6 bg-slate-50 border rounded-[30px] outline-none" value={apptForm.date} onChange={e => setApptForm({...apptForm, date: e.target.value})} /><button className="md:col-span-2 py-6 bg-indigo-600 text-white font-black rounded-[30px] shadow-2xl text-xl hover:bg-indigo-700 transition-all uppercase">Request Dr. {activeUser.doctorName}</button></form></div><div className="bg-white p-12 rounded-[50px] border shadow-sm uppercase font-black text-left font-sans shadow-sm"><h3 className="text-2xl mb-8 text-slate-400 uppercase font-black">My Requests</h3>{myAppointments.map((a: any) => <div key={a.id} className="p-6 bg-slate-50 border rounded-[30px] mb-4 flex justify-between items-center shadow-sm font-sans"><div><p className="text-lg font-black uppercase font-sans">{a.reason}</p><p className="text-[10px] text-indigo-400 mt-1 uppercase font-sans">Provider: {a.doctorDisplay}</p></div><p className="text-xs text-slate-400">{new Date(a.date).toLocaleString()}</p></div>)}</div></div>
            )}

            {activeTab === "history" && (
                <div className="bg-white p-12 rounded-[50px] border shadow-sm text-left uppercase font-black"><h3 className="text-2xl mb-10 tracking-widest uppercase">Intake Logs</h3>{(activeUser.medHistory || []).length === 0 ? <p className="text-center py-24 text-slate-300 italic tracking-widest text-lg opacity-40 uppercase">No history found.</p> : activeUser.medHistory.map((m:any, i:number) => (<div key={i} className="p-8 bg-emerald-50 border border-emerald-100 rounded-[35px] flex justify-between items-center mb-5 uppercase shadow-sm"><div><p className="text-2xl font-black">{m.name}</p><p className="text-[10px] text-slate-400 mt-1 uppercase">Scheduled: {m.time} | Confirmed At: {m.dateTaken}</p></div><CheckCircle2 className="text-emerald-500" size={40} /></div>))}</div>
            )}

            {activeTab === "patients" && (
              <div className="space-y-8 text-left uppercase font-black font-sans"><div className="bg-white p-12 rounded-[50px] border shadow-sm text-left font-black"><h3 className="text-2xl mb-8 tracking-widest uppercase font-black font-sans">Register Entry</h3><form onSubmit={(e) => { e.preventDefault(); if(!isPhoneValid(patientReg.phone)) return alert("Phone must be 10 digits."); if(!isBloodValid(patientReg.blood)) return alert("Invalid Blood Group."); setActiveUser({...activeUser, patients: [...(activeUser.patients || []), { ...patientReg, id: Date.now().toString() }] }); setPatientReg({ name: "", blood: "", phone: "", condition: "", famName: "", famPhone: "" }); alert("Record saved."); }} className="grid grid-cols-1 md:grid-cols-2 gap-8 font-black uppercase font-sans"><input required placeholder="Patient Full Name" className="p-6 bg-slate-50 border rounded-[30px] font-black outline-none" value={patientReg.name} onChange={e => setPatientReg({...patientReg, name: e.target.value})} /><input required placeholder="BLOOD GROUP" className="p-6 bg-slate-50 border rounded-[30px] uppercase outline-none font-black" value={patientReg.blood} onChange={e => setPatientReg({...patientReg, blood: e.target.value})} /><input required maxLength={10} placeholder="Mobile" className="p-6 bg-slate-50 border rounded-[30px] font-black outline-none" value={patientReg.phone} onChange={e => setPatientReg({...patientReg, phone: e.target.value})} /><input required placeholder="Diagnosis" className="p-6 bg-slate-50 border rounded-[30px] font-black outline-none" value={patientReg.condition} onChange={e => setPatientReg({...patientReg, condition: e.target.value})} /><input required placeholder="Backup Support Name" className="p-6 bg-slate-50 border rounded-[30px] font-black outline-none" value={patientReg.famName} onChange={e => setPatientReg({...patientReg, famName: e.target.value})} /><input required maxLength={10} placeholder="Backup Phone" className="p-6 bg-slate-50 border rounded-[30px] font-black outline-none" value={patientReg.famPhone} onChange={e => setPatientReg({...patientReg, famPhone: e.target.value})} /><button className="md:col-span-2 py-6 bg-indigo-600 text-white font-black rounded-[30px] shadow-2xl text-xl uppercase tracking-widest font-sans font-black">Confirm Record</button></form></div><div className="grid grid-cols-1 md:grid-cols-3 gap-8 uppercase">{(activeUser.patients || []).map((p:any) => (<div key={p.id} className="relative group text-left uppercase font-black"><div onClick={() => setSelectedPatient(p)} className={`p-10 rounded-[45px] border transition-all cursor-pointer shadow-sm ${selectedPatient?.id === p.id ? 'bg-indigo-600 text-white shadow-2xl' : 'bg-white hover:border-indigo-400'}`}><p className="font-black text-2xl mb-1 uppercase tracking-tighter text-left font-black">{p.name}</p><p className={`text-[10px] uppercase tracking-widest ${selectedPatient?.id === p.id ? 'text-white/70' : 'text-slate-400'}`}>{p.condition}</p></div><button onClick={(e) => { e.stopPropagation(); setActiveUser({...activeUser, patients: activeUser.patients.filter((x:any) => x.id !== p.id)}); if(selectedPatient?.id === p.id) setSelectedPatient(null); }} className="absolute -top-3 -right-3 p-4 bg-white text-rose-500 rounded-full shadow-2xl border-4 border-rose-50 hover:bg-rose-500 hover:text-white transition-all opacity-0 group-hover:opacity-100 transition-all font-black"><Trash2 /></button></div>))}</div>{selectedPatient && (<motion.div initial={{ y: 50 }} animate={{ y: 0 }} className="bg-indigo-900 text-white p-12 rounded-[60px] shadow-2xl space-y-10 uppercase font-black text-left font-black"><div className="flex justify-between items-start font-sans font-black"><div><h3 className="text-5xl tracking-tighter uppercase font-black font-sans">{selectedPatient.name}</h3><p className="font-bold opacity-70 mt-3 text-xl flex items-center gap-3 uppercase font-black font-sans"><Phone size={22} /> {selectedPatient.phone}</p></div><button onClick={() => setSelectedPatient(null)} className="px-8 py-3 bg-white/10 rounded-full text-xs border border-white/20 uppercase tracking-widest font-black">Close File</button></div><div className="grid grid-cols-1 md:grid-cols-2 gap-10 text-left uppercase font-black"><div className="bg-white/10 p-10 rounded-[50px] border border-white/10 text-left uppercase font-black"><p className="text-[10px] opacity-60 mb-3 tracking-widest font-sans font-black">History Details</p><p className="text-2xl font-medium leading-relaxed italic font-black font-sans">"{selectedPatient.condition}"</p><div className="mt-8 flex items-center gap-4 text-sm bg-indigo-800/50 w-fit px-6 py-3 rounded-3xl font-sans font-black"><Droplet size={18} className="text-rose-400"/> Blood Group {selectedPatient.blood}</div></div><div className="bg-white/10 p-10 rounded-[50px] border border-white/10 text-left font-black uppercase"><p className="text-[10px] opacity-60 mb-6 tracking-widest font-sans uppercase">Emergency Support</p><div className="flex items-center gap-6"><div className="w-16 h-16 rounded-[25px] bg-indigo-600 flex items-center justify-center shadow-lg font-black"><ShieldAlert size={35}/></div><div><p className="text-2xl font-black">{selectedPatient.famName}</p><p className="text-xl opacity-70 mt-1 uppercase font-sans font-black">{selectedPatient.famPhone}</p></div></div></div></div></motion.div>)}</div>
            )}

            {activeTab === "profile" && (
               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white p-12 rounded-[50px] border shadow-sm text-left uppercase space-y-12 font-black"><h2 className="text-4xl border-b pb-8 tracking-tighter uppercase font-black">My Profile</h2><div className="grid grid-cols-1 md:grid-cols-2 gap-12 font-black uppercase font-sans"><div><label className="text-[10px] text-slate-400 uppercase tracking-widest">USERNAME</label><p className="text-2xl mt-1 underline decoration-indigo-200 decoration-4 font-black">{activeUser.name}</p></div><div><label className="text-[10px] text-slate-400 uppercase tracking-widest font-black">PORTAL ROLE</label><p className="text-2xl text-indigo-600 mt-1 font-black uppercase">{activeUser.role}</p></div>{activeUser.role === 'patient' ? (<><div><label className="text-[10px] text-slate-400 uppercase tracking-widest font-black">CLINICAL DATA</label><p className="text-xl mt-1 font-black uppercase">{activeUser.age} Years • {activeUser.blood}</p></div><div><label className="text-[10px] text-rose-400 uppercase tracking-widest font-black">SOS CONTACT</label><p className="text-xl text-rose-600 mt-1 font-black flex items-center gap-2 uppercase font-sans font-black"><Phone size={18}/> {activeUser.emergencyName} ({activeUser.emergencyPhone})</p></div><div className="md:col-span-2 border-t pt-10 font-black"><label className="text-[10px] text-slate-400 uppercase tracking-widest font-black">ASSIGNED DOCTOR</label><p className="text-xl mt-2 font-black flex items-center gap-3 font-sans uppercase font-black font-black"><Stethoscope size={22} className="text-indigo-600"/> Dr. {activeUser.doctorName} • {activeUser.doctorPhone}</p></div></>) : (<div className="md:col-span-2 space-y-3 font-black"><label className="text-[10px] text-slate-400 uppercase tracking-widest font-black">FACILITY ADDRESS</label><p className="text-xl flex items-center gap-4 uppercase font-black font-sans font-black"><MapPin size={32} className="text-indigo-600 shrink-0"/> {activeUser.clinicAddress || 'Clinic Hub'}</p></div>)}</div><div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-20 font-black"><button onClick={handleLogout} className="w-full py-8 bg-slate-50 border-2 border-slate-200 text-slate-600 rounded-[35px] hover:bg-slate-200 transition-all uppercase tracking-widest font-black">Logout</button><button onClick={handleDeleteAccount} className="w-full py-8 bg-rose-50 border-2 border-rose-100 text-rose-600 rounded-[35px] hover:bg-rose-600 hover:text-white transition-all flex items-center justify-center gap-3 uppercase tracking-widest font-black">Delete Account Permanently</button></div></motion.div>
            )}
          </AnimatePresence>
        </div>
      </main>
